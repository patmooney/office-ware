...
export default class {
    ...
    */
                ...
                _this.schema.holiday.holidaysThisYear( req.user.id, org.reset_date ).then(
                    function ( days ) {
                        return res.json({ data: {
                            days: days,
                            remaining: req.user.days_allowed - days,
                            allowed: req.user.days_allowed
                        }});
                    }
                )
            }
    ...
    /* synchroniseUserHoliday
        Takes a user and works out their remaining holiday quota for the term
        according to the organisation reset date
        updates the user in the table and returns the remaining_days
    */
    synchroniseUserHoliday ( user ) {
        var _this = this;
        this.schema.organisation.find( user.organisation_id ).then(
            function ( org ) {
                _this.schema.holidays.holidaysThisYear( user.id, org.reset_date ).then(
                    function ( days ) {
                        _this.model.update(
                            {
                                day_remaining: user.allowed_days - days
                            },
                            {
                                where: {
                                    id: user.id
