...
export default function ( dsn ) {
    ...
    /* relationships */
    schema.holiday.User = schema.holiday.model.belongsTo(schema.user.model,{foreignKey: 'user_id'});
    schema.user.Holidays = schema.user.model.hasMany(schema.holiday.model,{foreignKey: 'user_id'});
    schema.user.Organisation = schema.user.model.belongsTo(schema.organisation.model,{foreignKey: 'organisation_id', as: 'Organisation'});
    schema.organisation.Users = schema.organisation.model.hasMany(schema.user.model,{foreignKey: 'organisation_id'});
